...
import localStyles from './enterChatRoomStyles';
...
export default function ChatScreen({ route, navigation }) { // 🌟 navigation prop을 받도록 추가 🌟
  ...
  const sendLeaveMessage = useCallback(() => {
    if (client.current && client.current.connected) {
      ...
      console.log(`${currentUser}님이 방 ${currentRoomId}에서 퇴장 메시지 보냄`);
    }
  ...
  useEffect(() => {
    client.current = new Client({
      ...
      onConnect: () => {
        ...
        client.current.subscribe(`/topic/chat/${currentRoomId}`, message => {
          ...
          if (!receivedMessage.id) {
              receivedMessage.id = Date.now().toString() + Math.random().toString(36).substr(2, 9);
          }
      ...
      onStompError: (frame) => {
        ...
        Alert.alert('연결 오류', '채팅 서버와 연결하는 데 문제가 발생했습니다.');
      },
      debug: (str) => {
  ...
  const inviteUser = () => {
      if (Platform.OS === 'web') { // 웹 환경일 경우
          const inviteeName = window.prompt('초대할 사용자의 닉네임 (ID)을 입력하세요.');
          if (inviteeName !== null) { // 사용자가 취소를 누르지 않았을 경우
              if (inviteeName.trim() !== '') {
                  if (client.current && client.current.connected) {
                      client.current.publish({
                      ...
                      console.log(`${currentUser}님이 ${inviteeName.trim()}님을 초대 메시지 보냄`);
                  }
              } else {
  ...
  const renderMessageItem = ({ item }) => {
    ...
    if (item.type !== MessageType.CHAT) {
      ...
      } else if (item.type === MessageType.LEAVE) { // 🌟 LEAVE 메시지 타입 추가
    ...
    return (
      <View style={[styles.messageContainer, isMyMessage ? styles.myMessage : styles.otherMessage]}>
        {!isMyMessage && <Text style={localStyles.senderText}>{item.sender}</Text>}
  ...
  return (
    ...
    >
      ...
      <View style={localStyles.headerContainer}>
        ...
        <Text style={localStyles.headerText}>방: {currentRoomName} (나: {currentUser})</Text>
      </View>
